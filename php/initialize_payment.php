...
function flutterwaveSecretKey($conn): string
{
    ...
    if ($secretKey === '') {
        $secretKey = trim((string)(getenv('FLW_SECRET_KEY') ?: ($_ENV['FLW_SECRET_KEY'] ?? '')));
    }
...
// DEBUG: LOG THE EXACT API RESPONSE TO A FILE
file_put_contents(__DIR__ . '/flutterwave_debug.log', date('Y-m-d H:i:s') . "\nHTTP CODE: " . $httpCode . "\nRESPONSE: " . $response . "\n\n", FILE_APPEND);
