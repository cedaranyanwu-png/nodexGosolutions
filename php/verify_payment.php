...
if (str_contains($secretKey, 'TEST-sandbox') || str_contains((string)$transactionId, 'mock_tr_') || str_contains((string)$transactionId, 'flw_tr_test_')) {
...
if ($paymentVerified &&
    ...
    // Extend from current subscription end if still active, or start fresh 1 month from now
    $currentSubEnd = !empty($user['subscription_end']) ? strtotime($user['subscription_end']) : 0;
    if ($currentSubEnd > time() && strtolower((string)($user['subscription_plan'] ?? '')) === strtolower((string)$payment['plan_name'])) {
        $subEnd = date('Y-m-d H:i:s', strtotime('+1 month', $currentSubEnd));
    } else {
        $subEnd = date('Y-m-d H:i:s', strtotime('+1 month'));
    }

    // 3. Persist the activated/upgraded premium plan state back into the users database
    $conn->update('users', [
    ...
    $conn->insert('activity_logs', [
        ...
        'details'    => "User activated/upgraded plan to '{$payment['plan_name']}' via verified Flutterwave transaction ref: {$txRef}",
