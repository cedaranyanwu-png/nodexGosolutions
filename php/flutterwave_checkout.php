...
// Legacy checkout route: never simulate success. Redirect only to a real hosted checkout link.
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $action = $_POST['action'] ?? 'pay';
    if ($action === 'pay' && !empty($payment['checkout_link']) && filter_var($payment['checkout_link'], FILTER_VALIDATE_URL)) {
        header('Location: ' . $payment['checkout_link']);
        exit;
    }
    header('Location: /user/dashboard?payment=' . rawurlencode($action === 'cancel' ? 'cancelled' : 'use_secure_checkout'));
    exit;
}
?>
<!DOCTYPE html>
