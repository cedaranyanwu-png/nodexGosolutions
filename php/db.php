...
if ($settingsCount === 0) {
    // Read keys dynamically from environment or local key file if available
    $flwSecret = getenv('FLW_SECRET_KEY') ?: '';
    $flwPublic = getenv('FLW_PUBLIC_KEY') ?: '';
    $flwEnc = getenv('FLW_ENCRYPTION_KEY') ?: '';

    $keysFilePath = __DIR__ . '/../flutterwave-keys.txt';
    if (file_exists($keysFilePath)) {
        $keysContent = (string)file_get_contents($keysFilePath);
        if (preg_match('/Secret Key:\s*(\S+)/i', $keysContent, $m)) {
            $flwSecret = $m[1];
        }
        if (preg_match('/Encryption Key:\s*(\S+)/i', $keysContent, $m)) {
            $flwEnc = $m[1];
        }
        if (preg_match('/Public Key:\s*(\S+)/i', $keysContent, $m)) {
            $flwPublic = $m[1];
        }
    }
    ...
    $conn->insert('settings', [
        'id' => 'flutterwave',
        'flw_public_key' => $flwPublic ?: 'FLWPUBK_TEST-sandbox-pubkey-123456789',
        'flw_secret_key' => $flwSecret ?: 'FLWSECK_TEST-sandbox-secretkey-123456789',
        'flw_encryption_key' => $flwEnc ?: 'FLWENCK_TEST-sandbox-encryptionkey-123456789',
        'updated_at' => date('Y-m-d H:i:s')
    ]);
}
