...
// Load centralized runtime constants before any service resolves tenant paths.
...
if (!$existingSettings) {
    ...
    // Replace any legacy seeded sandbox values with securely discovered live values,
    // or clear them when no live credentials are configured.
    $currentSecret = (string)($existingSettings['flw_secret_key'] ?? '');
    if (str_contains($currentSecret, 'TEST-sandbox')) {
 ...
 */
function tenantWebsiteUrl(array|string $websiteOrSubdomain): string {
    $subdomain = is_array($websiteOrSubdomain)
        ? (string)($websiteOrSubdomain['subdomain'] ?? '')
        : (string)$websiteOrSubdomain;
    $subdomain = strtolower(trim($subdomain));
    $subdomain = preg_replace('/[^a-z0-9-]/', '', $subdomain) ?? '';

    $requestHost = strtolower((string)($_SERVER['HTTP_HOST'] ?? ''));
    $requestHostWithoutPort = explode(':', $requestHost)[0];
    $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';

    // Production links are host-based: https://tenant.nodexplatform.com.ng/
    $mainDomain = defined('MAIN_DOMAIN') ? strtolower((string)MAIN_DOMAIN) : 'nodexplatform.com.ng';
    if ($requestHostWithoutPort === 'localhost' || str_ends_with($requestHostWithoutPort, '.localhost')) {
        $port = str_contains($requestHost, ':') ? ':' . explode(':', $requestHost, 2)[1] : '';
        return $scheme . '://' . $subdomain . '.localhost' . $port . '/';
    }
