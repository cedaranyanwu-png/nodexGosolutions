 ...
 * sidebar.php
 *
 * Upgraded modular sidebar navigation component shared by both the Admin and User dashboards.
 ...
 * Compatible with SPA dashboard.php and direct tab file access.
 */
...
$sidebarAvatar = trim((string)($sidebarUser['avatar'] ?? ''));

// Detect if we are inside the dashboard context (SPA or direct tab access)
$isDashboardPage = isset($dashboardContext) || str_contains($_SERVER['REQUEST_URI'], 'dashboard');
$currentTab = $activeTab ?? $requestedTab ?? '';
?>
...
<div class="sidebar-container d-flex flex-column flex-shrink-0 p-3 bg-white border-end" id="sidebarContainer" style="width: 260px; min-width: 260px; max-width: 260px; min-height: 100vh; border-color: rgba(0, 114, 255, 0.1) !important; box-shadow: 4px 0 12px rgba(0, 114, 255, 0.03); z-index: 1050; transition: transform 0.3s ease;">
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
        <a href="/" class="nx-secure-brand d-flex align-items-center text-decoration-none" aria-label="NodeXGo home">
            <img src="/main/assets/images/logo.png" alt="NodeXGo" class="nx-secure-logo me-2" style="max-height: 38px; max-width: 180px; width: auto; object-fit: contain;">
            <span class="nx-secure-brand-copy"><strong>NodeXGo</strong><small><?php echo isStaff() ? 'Operations Console' : 'Workspace Console'; ?></small></span>
        </a>
    ...
    <div class="d-flex align-items-center px-2 mb-4">
        <?php if ($sidebarAvatar): ?>
            <img src="<?php echo htmlspecialchars($sidebarAvatar); ?>" class="rounded-circle object-fit-cover border border-2 border-primary" style="width: 40px; height: 40px;" alt="Profile picture of <?php echo htmlspecialchars($fullname); ?>">
        <?php else: ?>
            <div class="rounded-circle d-flex align-items-center justify-content-center text-white fw-bold bg-primary" style="width: 40px; height: 40px; font-size: 16px;">
                <?php echo htmlspecialchars(strtoupper(substr($fullname, 0, 1))); ?>
            </div>
        <?php endif; ?>
    ...
    <div class="overflow-y-auto flex-grow-1">
        <ul class="nav nav-pills flex-column gap-1 list-unstyled mb-auto">
            ...
            <li class="nav-item">
                ...
                <?php else: ?>
                    <a href="/user/dashboard" class="nav-link d-flex align-items-center py-2 px-3 fw-bold rounded-pill text-dark hover-blue <?php echo ($isDashboardPage && empty($currentTab)) ? 'active text-white bg-primary' : ''; ?>">
                ...
                <div class="accordion accordion-flush" id="sidebarToolsAccordion" style="--bs-accordion-bg: transparent;">
                    ...
                    <?php if (!isStaff()): ?>
                        ...
                        <div class="accordion-item border-0">
                            ...
                            <div id="collapseWebsiteTools" class="accordion-collapse collapse" data-bs-parent="#sidebarToolsAccordion">
                                <div class="accordion-body py-1 ps-4 pe-2 d-flex flex-column gap-1">
                                    ...
                                    <a href="/user/dashboard#manage-files" class="nav-link d-flex align-items-center py-1.5 px-3 rounded-pill text-muted hover-blue text-xs fw-semibold <?php echo ($currentTab === 'manage-files') ? 'active text-white bg-primary' : ''; ?>">
                                        <i class="fa-solid fa-folder-open me-2" style="font-size: 11px;"></i> Manage Files
                                    </a>
                                    <a href="/user/dashboard#monetization" class="nav-link d-flex align-items-center py-1.5 px-3 rounded-pill text-muted hover-blue text-xs fw-semibold <?php echo ($currentTab === 'monetization') ? 'active text-white bg-primary' : ''; ?>">
                                        <i class="fa-solid fa-rectangle-ad me-2" style="font-size: 11px;"></i> Monetization
                                    </a>
                                    <a href="/user/dashboard#analytics" class="nav-link d-flex align-items-center py-1.5 px-3 rounded-pill text-muted hover-blue text-xs fw-semibold <?php echo ($currentTab === 'analytics') ? 'active text-white bg-primary' : ''; ?>">
                                        <i class="fa-solid fa-chart-simple me-2" style="font-size: 11px;"></i> Analytics
                                    </a>
                                    <a href="/user/dashboard#profile" class="nav-link d-flex align-items-center py-1.5 px-3 rounded-pill text-muted hover-blue text-xs fw-semibold <?php echo ($currentTab === 'profile') ? 'active text-white bg-primary' : ''; ?>">
                                        <i class="fa-solid fa-user-gear me-2" style="font-size: 11px;"></i> My Profile
                                    </a>
                                    <a href="/user/dashboard#team" class="nav-link d-flex align-items-center py-1.5 px-3 rounded-pill text-muted hover-blue text-xs fw-semibold <?php echo ($currentTab === 'team') ? 'active text-white bg-primary' : ''; ?>">
                                        <i class="fa-solid fa-users-gear me-2" style="font-size: 11px;"></i> Workspace Team
                                    </a>
                                    <a href="/user/dashboard#workspace-settings" class="nav-link d-flex align-items-center py-1.5 px-3 rounded-pill text-muted hover-blue text-xs fw-semibold <?php echo ($currentTab === 'workspace-settings') ? 'active text-white bg-primary' : ''; ?>">
                        ...
                        <div class="accordion-item border-0 mt-1">
                            ...
                            <div id="collapseDatabaseTools" class="accordion-collapse collapse" data-bs-parent="#sidebarToolsAccordion">
                                <div class="accordion-body py-1 ps-4 pe-2 d-flex flex-column gap-1">
                                    <a href="/user/dashboard#user-db-manager" class="nav-link d-flex align-items-center py-1.5 px-3 rounded-pill text-muted hover-blue text-xs fw-semibold <?php echo ($currentTab === 'user-db-manager') ? 'active text-white bg-primary' : ''; ?>">
                    ...
                    <?php else: ?>
                        ...
                        $adminSidebarItems = [
                            ...
                            ['label' => 'File Workspace', 'icon' => 'fa-solid fa-folder-tree', 'url' => '/admin/files'],
                    ...
                    <?php
                    if (file_exists(__DIR__ . '/../../php/SystemDiscovery.php')) {
                        require_once __DIR__ . '/../../php/SystemDiscovery.php';
                        $sysDiscovery = new SystemDiscovery();
                        $sidebarDiscovered = $sysDiscovery->getByPlacement('sidebar', $activeRole);
                        if (!empty($sidebarDiscovered)):
                    ?>
                        <div class="accordion-item border-0 mt-1">
                            <div class="accordion-header">
                                <button class="accordion-button collapsed py-2 px-3 fw-bold text-dark rounded-pill hover-blue shadow-none d-flex align-items-center" type="button" data-bs-toggle="collapse" data-toggle="collapse" data-bs-target="#collapseSystemDiscovered" data-target="#collapseSystemDiscovered" aria-expanded="false" aria-controls="collapseSystemDiscovered" style="background: transparent; font-size: 14px;">
                                    <i class="fa-solid fa-cubes me-2 text-primary" style="width: 20px;"></i>
                                    System Apps & Tools
                                </button>
                            </div>
                            <div id="collapseSystemDiscovered" class="accordion-collapse collapse" data-bs-parent="#sidebarToolsAccordion">
                                <div class="accordion-body py-1 ps-4 pe-2 d-flex flex-column gap-1">
                                    <?php foreach ($sidebarDiscovered as $sTool):
                                        $dashPath = isStaff() ? '/admin/dashboard' : '/user/dashboard';
                                        $targetUrl = !empty($sTool['url']) ? $sTool['url'] : ($dashPath . '#tool-' . htmlspecialchars((string)($sTool['slug'] ?? '')));
                                        $toolIcon = htmlspecialchars((string)($sTool['icon'] ?? 'fa-solid fa-cube'));
                                        $toolLabel = htmlspecialchars((string)($sTool['name'] ?? ''));
                                    ?>
            ...
            <li>
                <?php if (isStaff()): ?>
                    <a href="/admin/profile" class="nav-link d-flex align-items-center py-2 px-3 fw-bold text-dark rounded-pill hover-blue <?php echo str_contains($_SERVER['REQUEST_URI'], '/admin/profile') ? 'active text-white bg-primary' : ''; ?>">
                        <i class="fa-solid fa-user me-2" style="width: 20px;"></i>
                        Profile Settings
                    </a>
                <?php else: ?>
                    <a href="/user/dashboard#profile" class="nav-link d-flex align-items-center py-2 px-3 fw-bold text-dark rounded-pill hover-blue <?php echo ($currentTab === 'profile') ? 'active text-white bg-primary' : ''; ?>">
                        <i class="fa-solid fa-user me-2" style="width: 20px;"></i>
                        Profile Settings
                    </a>
                <?php endif; ?>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const autoExpandActiveDropdown = function() {
        ...
        const targetMap = {
            ...
            '#team': 'collapseWebsiteTools',
            '#workspace-settings': 'collapseWebsiteTools',
