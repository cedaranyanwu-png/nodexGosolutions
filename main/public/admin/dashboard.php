...
// Initialize dynamic alert status notification variables for admin profile updates
$profileSuccess = '';
$profileError = '';

// Handle administrator profile and password updates when submitted via POST
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'update_profile') {
        ...
        if (isset($_FILES['avatar_file']) && $_FILES['avatar_file']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($fileExtension, $allowedExtensions, true)) {
                ...
                if (move_uploaded_file($fileTmpPath, $destPath)) {
                    ...
                    $profileError = 'Failed to move uploaded avatar file.';
                }
            } else {
                $profileError = 'Invalid image extension allowed. Choose webp, png, jpeg, or gif.';
            }
        ...
        if (empty($fullname)) {
            $profileError = 'Full name field cannot be empty.';
        }

        // If no processing errors occurred, commit changes to system
        if (empty($profileError)) {
            // Update administrator information inside custom JSON Database
            $conn->update('users', [
                'fullname' => $fullname,
                'avatar' => $avatarUrl,
            ...
            $user = $conn->selectOne('users', ['email' => $_SESSION['email']]);
            $profileSuccess = 'Profile information successfully updated!';
        }
    }
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'update_password') {
        ...
        if (empty($currentPassword) || empty($newPassword) || empty($confirmPassword)) {
            $profileError = 'All password fields are strictly required.';
        } elseif ($newPassword !== $confirmPassword) {
            $profileError = 'New password and confirmation fields do not match.';
        } elseif (strlen($newPassword) < 6) {
            $profileError = 'New password must be at least 6 characters in length.';
        } else {
            ...
            if (password_verify($currentPassword, $user['password'])) {
                ...
                $profileSuccess = 'Your password has been changed successfully!';
            } else {
                $profileError = 'Invalid current password entered.';
            }
        }
    }
}
...
<div class="dashboard-layout">
  ...
  <div class="main-content">
    ...
    <ul class="nav nav-tabs border-0 flex-wrap gap-2 mb-4" id="adminTabs" role="tablist">
      ...
      <!-- Tab 10: Platform Analytics - Accessible dynamically inside the dashboard -->
      <li class="nav-item" role="presentation">
        <button class="nav-link border-0 <?php echo $activeTab === 'analytics' ? 'active' : ''; ?>" id="analytics-tab" data-bs-toggle="tab" data-bs-target="#analytics-section" type="button" role="tab">Platform Analytics</button>
      </li>

      <!-- Tab 11: Profile Settings - Secure admin credentials and details modification forms -->
      <li class="nav-item" role="presentation">
        <button class="nav-link border-0 <?php echo $activeTab === 'profile' ? 'active' : ''; ?>" id="profile-tab" data-bs-toggle="tab" data-bs-target="#profile-section" type="button" role="tab">Profile Settings</button>
      </li>
    </ul>
    ...
    <div class="tab-content" id="adminTabContent">
      ...
      <!-- TAB 11: PLATFORM ANALYTICS CONSOLE -->
      <div class="tab-pane fade <?php echo $activeTab === 'analytics' ? 'show active' : ''; ?>" id="analytics-section" role="tabpanel" aria-labelledby="analytics-tab">
        <div class="card border-0 shadow-sm rounded-xl mb-4">
          <div class="card-header bg-white border-b border-gray-100 py-3.5 px-4">
            <h3 class="text-base font-bold text-gray-800 m-0"><i class="fas fa-chart-simple text-blue-600 mr-2"></i> Platform-Wide Traffic Analytics</h3>
          </div>
          <div class="card-body p-4">
            <?php
            // Calculate system-wide cumulative statistics dynamically
            $conn->createTable('traffic');
            $allTraffic = $conn->select('traffic') ?: [];
            $allWebs = $conn->select('websites') ?: [];
            ...
            foreach ($allTraffic as $t) {
                $totalPageViews += (int)($t['page_views'] ?? 0);
                $totalClicks += (int)($t['clicks'] ?? 0);
                $totalVisits += (int)($t['visits'] ?? 0);
                $totalVisitors += (int)($t['visitors'] ?? 0);
            }
            ?>
            <!-- Stats cards grids -->
            <div class="row mb-4">
              ...
              <div class="col-lg-3 col-md-6 col-12 mb-3">
                <div class="bg-gray-50 border border-gray-100 rounded-xl p-4 flex justify-between items-center shadow-2xs">
                  <div>
                    <span class="text-2xs font-bold text-gray-400 uppercase tracking-wider block mb-1">Cumulative Page Views</span>
                    <span class="text-xl font-extrabold text-gray-800"><?php echo number_format($totalPageViews); ?></span>
                  </div>
                  <div class="w-10 h-10 bg-primary bg-opacity-10 text-primary rounded-xl flex items-center justify-center text-lg">
                    <i class="fas fa-eye"></i>
                  </div>
                </div>
              </div>
              <!-- Cumulative Click Interactions -->
              <div class="col-lg-3 col-md-6 col-12 mb-3">
                <div class="bg-gray-50 border border-gray-100 rounded-xl p-4 flex justify-between items-center shadow-2xs">
                  <div>
                    <span class="text-2xs font-bold text-gray-400 uppercase tracking-wider block mb-1">Click Interactions</span>
                    <span class="text-xl font-extrabold text-blue-600"><?php echo number_format($totalClicks); ?></span>
                  </div>
                  <div class="w-10 h-10 bg-info bg-opacity-10 text-info rounded-xl flex items-center justify-center text-lg">
              ...
              <div class="col-lg-3 col-md-6 col-12 mb-3">
                <div class="bg-gray-50 border border-gray-100 rounded-xl p-4 flex justify-between items-center shadow-2xs">
                  <div>
                    <span class="text-2xs font-bold text-gray-400 uppercase tracking-wider block mb-1">Total Visits</span>
                    <span class="text-xl font-extrabold text-gray-800"><?php echo number_format($totalVisits); ?></span>
                  </div>
                  <div class="w-10 h-10 bg-success bg-opacity-10 text-success rounded-xl flex items-center justify-center text-lg">
              ...
              <div class="col-lg-3 col-md-6 col-12 mb-3">
                <div class="bg-gray-50 border border-gray-100 rounded-xl p-4 flex justify-between items-center shadow-2xs">
                  <div>
                    <span class="text-2xs font-bold text-gray-400 uppercase tracking-wider block mb-1">Unique Visitors</span>
                    <span class="text-xl font-extrabold text-gray-800"><?php echo number_format($totalVisitors); ?></span>
                  </div>
                  <div class="w-10 h-10 bg-warning bg-opacity-10 text-warning rounded-xl flex items-center justify-center text-lg">
            ...
            <!-- Websites detailed metrics directory -->
            <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Tenant Websites Traffic Analytics</h4>
            <div class="table-responsive">
              <table class="table table-hover mb-0 text-xs">
                <thead class="bg-gray-50 text-gray-500 font-bold">
                  <tr>
                    <th class="p-3">Website Title</th>
                    <th class="p-3">Subdomain Address</th>
                    <th class="p-3 text-center">Visits</th>
                    <th class="p-3 text-center">Page Views</th>
                    <th class="p-3 text-center">Clicks</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                  <?php if (count($allWebs) > 0): ?>
                    <?php foreach ($allWebs as $wb):
                        $wtRow = $conn->selectOne('traffic', ['website_id' => $wb['id']]);
                        $wVisits = (int)($wtRow['visits'] ?? 0);
                        $wViews = (int)($wtRow['page_views'] ?? 0);
                        $wClicks = (int)($wtRow['clicks'] ?? 0);
                    ?>
                      <tr>
                        <td class="p-3 font-semibold text-gray-800"><?php echo htmlspecialchars((string)($wb['name'] ?? '')); ?></td>
                        <td class="p-3 font-mono text-blue-600"><?php echo htmlspecialchars((string)($wb['subdomain'] ?? '')); ?>.<?php echo htmlspecialchars($hostDomain); ?></td>
                        <td class="p-3 text-center font-bold text-gray-800"><?php echo number_format($wVisits); ?></td>
                        <td class="p-3 text-center text-gray-700"><?php echo number_format($wViews); ?></td>
                        <td class="p-3 text-center font-semibold text-blue-600"><?php echo number_format($wClicks); ?></td>
                      </tr>
                    <?php endforeach; ?>
                  <?php else: ?>
                    <tr>
                      <td colspan="5" class="text-center text-gray-400 py-6">No websites deployed on the platform yet.</td>
                    </tr>
                  <?php endif; ?>
                </tbody>
              </table>
      ...
      <!-- TAB 12: PROFILE SETTINGS CONSOLE -->
      <div class="tab-pane fade <?php echo $activeTab === 'profile' ? 'show active' : ''; ?>" id="profile-section" role="tabpanel" aria-labelledby="profile-tab">
        <!-- Render status notification alerts if any -->
        <?php if (!empty($profileSuccess)): ?>
          <div class="alert alert-success text-xs rounded-lg p-3 mb-4 d-block" role="alert">
              <i class="fas fa-circle-check me-2"></i><strong>Success:</strong> <?php echo htmlspecialchars((string)$profileSuccess); ?>
          </div>
        <?php endif; ?>
        <?php if (!empty($profileError)): ?>
          <div class="alert alert-danger text-xs rounded-lg p-3 mb-4 d-block" role="alert">
              <i class="fas fa-exclamation-triangle me-2"></i><strong>Error:</strong> <?php echo htmlspecialchars((string)$profileError); ?>
          </div>
        <?php endif; ?>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div class="lg:col-span-1">
                <div class="card border-0 shadow-sm rounded-xl p-4 bg-white text-center">
                  <div class="mb-4">
                    <!-- Render dynamic administrator avatar image -->
                    <?php if (!empty($user['avatar'])): ?>
                      <img src="<?php echo htmlspecialchars((string)$user['avatar']); ?>" alt="User Avatar" class="w-24 h-24 rounded-full mx-auto object-cover border-4 border-blue-500 shadow-md">
                    <?php else: ?>
                      <div class="w-24 h-24 rounded-full mx-auto bg-blue-600 text-white font-bold text-3xl flex items-center justify-center border-4 border-blue-100 shadow-md">
                        <?php echo strtoupper(substr((string)($user['fullname'] ?? 'A'), 0, 1)); ?>
                      </div>
                    <?php endif; ?>
                  </div>
                  ...
                  <div class="bg-blue-50 text-blue-700 text-xs font-semibold py-1 px-3 rounded-full inline-block uppercase tracking-wider">
                    <?php echo htmlspecialchars(strtoupper((string)($user['role'] ?? 'admin'))); ?>
                  </div>
                </div>
              </div>

              <!-- Right Column: Modification Forms -->
              <div class="lg:col-span-2 flex flex-col gap-6">

                <!-- Account details form card -->
                <div class="card border-0 shadow-sm rounded-xl bg-white">
                  <div class="card-header bg-white border-b border-gray-100 py-3.5 px-4">
                    <h4 class="text-sm font-bold text-gray-800 m-0"><i class="fas fa-user-circle text-primary mr-2"></i> Update Administrative Information</h4>
                  </div>
                  <div class="card-body p-4">
                    <form action="/admin/dashboard#profile" method="POST" enctype="multipart/form-data">
                      <input type="hidden" name="action" value="update_profile" />

                      <div class="mb-3.5">
                        <label class="block text-xs font-bold text-gray-500 uppercase mb-2">Administrator Full Name</label>
                ...
                <div class="card border-0 shadow-sm rounded-xl bg-white">
                  <div class="card-header bg-white border-b border-gray-100 py-3.5 px-4">
                    <h4 class="text-sm font-bold text-gray-800 m-0"><i class="fas fa-key text-red-500 mr-2"></i> Update Security Credentials</h4>
                  </div>
                  <div class="card-body p-4">
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
      <div class="modal-content rounded-xl border-0 shadow-2xl">
        <div class="modal-header border-b border-gray-100 bg-slate-900 text-white rounded-t-xl py-3">
          <h5 class="modal-title font-bold flex items-center text-sm" id="adminTicketReplyModalLabel"><i class="fa-solid fa-headset mr-2 text-emerald-400"></i> Manage Support Ticket</h5>
...
$(document).ready(function() {
    ...
    const handleAdminHashes = function() {
        ...
        // Map hash tags to active tab button IDs for all administrative features
        const tabMap = {
            ...
            '#analytics-section': 'analytics-tab',
            '#profile': 'profile-tab',
